...
// Wrapper tatt fra https://github.com/audunel/TTK4145/blob/master/project/driver/driver.go
// og redigert litt
...
func DriverInit(buttonEvent chan def.OrderButton, floorEvent chan int) {
	...
	for {
		...
		if floorSignal != lastPassedFloor && floorSignal != -1 {
			...
			for button := def.ButtonCallDown; int(button) < def.NumButtons; button++ {
